...
@Component
public class SatelliteRestItemProcessor implements ItemProcessor<AppointmentModel, JobExecution> {
	...
	@Override
	public JobExecution process(AppointmentModel item) throws Exception {
		LOGGER.info("Inside Processor");
		LOGGER.info("Printing value in processor " + item.getPatientFirstName());
		...
		LOGGER.info("Checking list size" + requestList.size());
		return jobExecution;
	}
